...
trait MacOsCpuInfoCommand {
	...
	def numProcessors =  parseLineFor("machdep.cpu.core_count")
	def cpuCoresPerProcessor = parseLineFor("machdep.cpu.cores_per_package")
	private def parseLineFor(key:String) = cpuInfo.filter(x => x.startsWith(key)).last.split(":").last.trim.toInt
}
